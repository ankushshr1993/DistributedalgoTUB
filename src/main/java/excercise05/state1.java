...
public class state1 extends Node {
    ...
    public void test() {
     ...
     /*   if (gotToken) {
        ...
        //seqNum++;
        RN[me] = seqNum++;
        Message message= new Message(seqNum +me + LN[me]);
        sendAll(message);
    ...
    public void onMessage(Message message) {
        ...
        this.ancestor = message.getSender();
        for (i=0,i=this.getNeighbors().size(),i++)
        { tokenid[]=ancestor;
        RN[i]=seqNum;
            if(gotToken= true) {
                 RN[i] = Math.max(RN[i], seqNum);
                 Message tokenmsg= new Message(tokenMSG);
                 send(ancestor,tokenmsg);
            }
